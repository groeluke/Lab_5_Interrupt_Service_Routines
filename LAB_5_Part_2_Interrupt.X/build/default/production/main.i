...
Setup:
    ...
    ; Bank 1
    BANKSEL TRISA
    BCF TRISA,0 ;Set ((PORTA) and 07Fh), 0 as output for monitoring
    CLRF TRISC ;PORTC all outputs (LEDs)
    BANKSEL OPTION_REG
    BCF OPTION_REG,6 ;will be on a falling edge
    BSF IOCB,4 ;enable interrupt on change for ((PORTB) and 07Fh), 4
...
Main:
    MOVLW 0x01 ;Loads ((PORTA) and 07Fh), 0 to toggle
    XORWF PORTA,1 ;toggle ((PORTA) and 07Fh), 0
    GOTO Main
...
ISRHandler:
    ...
    ; check ((PORTB) and 07Fh), 0
    BTFSS INTCON, 1 ;check if it was ((PORTB) and 07Fh), 0 (((INTCON) and 07Fh), 1 set?)
    GOTO CheckIOC ;no ? check the other source

    ; service ((PORTB) and 07Fh), 0
    CLRF PORTC ;clear count
    BCF INTCON, 1 ;clear external interrupt flag
    GOTO ISRExit

CheckIOC:
    ; check ((PORTB) and 07Fh), 4
    BTFSS INTCON, 0 ;check if it was ((PORTB) and 07Fh), 4 (((INTCON) and 07Fh), 0 set?)
    GOTO ISRExit ;neither flag ? just exit

    ; service ((PORTB) and 07Fh), 4 IOC
    MOVF PORTB, W ;read PORTB to end the mismatch condition
    INCF PORTC, F ;count one press
