...
ISRHandler:
    ...
    BTFSS INTCON,0 ;((INTCON) and 07Fh), 0 set?
    ...
    BTFSC RB_CHANGED,7
    GOTO Pin7
    GOTO IOC_Done ;PB_CHANGED was 0 -- shouldn't happen, but safe
...
SetPin:
    MOVWF PORTA ;display the 3-bit pin number

IOC_Done:
    BCF INTCON,0 ;clear ((INTCON) and 07Fh), 0 -- safe now, PORTB was already re-read
